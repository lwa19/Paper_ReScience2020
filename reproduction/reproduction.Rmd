...
*LW: I understand resetting the wd in each code chunk is necessary for
the R scripts sourced to run correctly, but I still think modifying
those R scripts to all use the same reference wd ('/reproduction in
this case') would make it a lot more clear and consistent.*
...
*LW: since MCMC here takes a good while to run, I think it might be
helpful to include a note reminding the people using the code or
include a sample run time so people have a rough idea of how long it
might take.*
...
*LW: I got confused when I was running through it because the first
table generated here is in fact supplement table 6 and not table 3. I
think the legend of table 2 in the 2020 paper if added here would make
a lot more sense about the distinction between table 3 and table s6.*
